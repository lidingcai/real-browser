import { app, ipcMain as ipc, session, WebContents } from 'electron'

import { sendIPCToWindow } from './main'
import { getTabIDFromWebContents } from './viewManager'
...
let pendingPermissions: PermissionType[] = []
 ...
 */
function hasPendingRequestForOrigin(requestOrigin, permission, _details) {
...
function pagePermissionRequestHandler(webContents, permission, callback, details) {
  ...
  */
  if (['media', 'notifications', 'pointerLock'].includes(permission)) {
    ...
    */
    webContents.on('did-start-navigation', (_e, _url, isInPlace, isMainFrame, _frameProcessId, _frameRoutingId) => {
...
function pagePermissionCheckHandler(_webContents, permission, requestingOrigin, details) {
...
export const initPermissionManager = () => {
  app.once('ready', () => {
  ...
  ipc.on('permissionGranted', (_e, permissionId) => {
    for (let i = 0; i < pendingPermissions.length; i++) {
