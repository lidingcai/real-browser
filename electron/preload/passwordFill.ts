...
export const initPasswordFill = () => {
  ...
  function createUnlockButton(input: Element) {
    ...
    button.innerHTML = keyIcon as unknown as string

    // Button hover.
    button.addEventListener('mouseenter', (_event) => {
      button.style.opacity = '1.0'
    })
    button.addEventListener('mouseleave', (_event) => {
      button.style.opacity = '0.7'
    })
  ...
  function addFocusListener(element: HTMLInputElement, credentials) {
    const inputRect = element.getBoundingClientRect()
    // Creates an options list container.
    function buildContainer() {
      const suggestionsDiv = document.createElement('div')
      // suggestionsDiv.style =
      //  'position: absolute; border: 1px solid #d4d4d4; z-index: 999999; border-bottom: none; background: #FFFFFF;
      // transform: scale(0); opacity: 0; transform-origin: top left; transition: 0.15s; color: #000000;'
      suggestionsDiv.style.position = 'absolute'
      suggestionsDiv.style.border = '1px solid #d4d4d4'
      suggestionsDiv.style.zIndex = '999999'
      suggestionsDiv.style.borderBottom = 'none'
      suggestionsDiv.style.background = '#FFFFFF'
      suggestionsDiv.style.transform = 'scale(0)'
      suggestionsDiv.style.opacity = '0'
      suggestionsDiv.style.transformOrigin = 'top left'
      suggestionsDiv.style.transition = '0.15s'
    ...
    function addOption(parent, username) {
      ...
      suggestionItem.style.borderBottom = '1px solid #d4d4d4'

      // Hover.
      suggestionItem.addEventListener('mouseenter', (_event) => {
        suggestionItem.style.backgroundColor = '#e4e4e4'
      })
      suggestionItem.addEventListener('mouseleave', (_event) => {
        suggestionItem.style.backgroundColor = '#fff'
      })

      // When user clicks on the suggestion, we populate the form inputs with selected credentials.
      suggestionItem.addEventListener('click', (_e) => {
  ...
  function maybeAddUnlockButton(target: Element) {
  ...
  ipc.on('password-autofill-match', (_event, data) => {
  ...
  ipc.on('password-autofill-shortcut', (_event) => {
    requestAutofill()
  })

  // Autofill enabled event handler. Initializes focus listeners for input fields.
  ipc.on('password-autofill-enabled', (_event) => {
    checkInitialFocus()
  ...
  window.addEventListener(
    'click',
    (e) => {
      const path = e.composed && e.composedPath()
      if (!path) {
        return
      }
      ;(path as HTMLButtonElement[]).forEach((el) => {
