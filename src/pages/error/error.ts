...
export const initError = () => {
  ...
  const errDesc = erorDescriptions[ec as keyof typeof erorDescriptions] as {
  ...
  if (errDesc) {
    ...
    subtitle = `${errorCodes[ec as keyof typeof errorCodes] || ''} (${ec})`
  }
  ...
  if (errDesc && errDesc.secondaryAction) {
    ...
    secondaryButton!.addEventListener('click', () => {
      window.location.href = errDesc.secondaryAction.url
    })
  }

  // if an ssl error occured, "try again" should go to the http:// version, which might work

  if (erorDescriptions[ec as keyof typeof erorDescriptions] === sslError) {
